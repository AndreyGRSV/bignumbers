...
class bdig {
    ...
    bdig& div3(const bdig& v, bdig* premainder = NULL)
    {
        ...
        while (true) {
            ...
            int msb_x = (isz - msi - 1) * std::numeric_limits<T>::digits
                + most_significant_bit(integer[msi]);
            msi = isz - v.most_significant_index() - 1;
            int msb_y = (isz - msi - 1) * std::numeric_limits<T>::digits
                + most_significant_bit(v.integer[msi]);
    ...
    bdig& div2(const bdig& v, bdig* premainder = NULL)
    {
        ...
        int msi = most_significant_index();
        int bits
            = (isz - msi - 1) * std::numeric_limits<T>::digits + most_significant_bit(integer[msi]);

        int v_bits = (isz - v.most_significant_index() - 1) * std::numeric_limits<T>::digits
            + most_significant_bit(v.integer[v.most_significant_index()]);
    ...
    template<class Ti>
    bdig& mul(
        const bdig& v, typename enable_if<is_same<Ti, unsigned long long>::value, Ti>::type = 0)
    {
    ...
    template<class Ti>
    Ti _toll(typename enable_if<std::numeric_limits<Ti>::is_integer, Ti>::type = 0)
    {
        Ti result = 0;
...
for (std::vector<std::string>::reverse_iterator it = data.rbegin(); it
!= data.rend(); ++it) output << *it << std::endl;

//int msi = rhs.most_significant_index();
//output.write ((std::ostream::char_type *)&rhs.integer[msi], (isz -
msi) * sizeof(T) / sizeof (std::ostream::char_type)); return output;
}
...
try
{
        ...
        out.write ((std::ostream::char_type *)&integer[0], isz *
sizeof(T) / sizeof (std::ostream::char_type)); out.close();
}
...
try
{
        ...
        in.seekg( -( isz * sizeof(T) / sizeof (std::ifstream::char_type)
), in.end); std::streamoff pos = in.tellg(); in.read((std::ifstream::char_type
*)integer, isz * sizeof(T) / sizeof (std::ifstream::char_type)); in.close();
}
catch (std::ios_base::failure f)
{
