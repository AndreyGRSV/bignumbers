/*
	Template class of big numbers with fixed point arithmetic for simplifying usage of big numbers.
	Usage from C++98.
...
namespace sag
{
	...
	template <const int digits = 100, const int prec = 0, class T = unsigned char> class bdig {
		static const typename enable_if <!std::numeric_limits<T>::is_signed, unsigned int>::type isz = ( (prec * 2 + digits) / (std::numeric_limits<T>::digits  * 301 / 1000) ) + 1;
		...
		template <class Ti> int most_significant_bit(Ti value, typename enable_if< is_same<Ti, unsigned char>::value && std::numeric_limits<Ti>::digits == 8, T>::type = 0) const
		{
			if ( value & (Ti)0xF0 )
			{
				if ( value & (Ti)0xC0 )
		...
		bdig& copy (const bdig& src, int bit_most, int bit_least)
		{
			...
			int mask = std::numeric_limits<T>::max() >> (std::numeric_limits<T>::digits - (bit_most + 1) % std::numeric_limits<T>::digits);
		...
		bdig& div3 (const bdig& v, bdig *premainder = NULL)
		{
			...
			while (true)
			{
				int msi = isz - most_significant_index() - 1;
				int msb_x = (isz - msi - 1) * std::numeric_limits<T>::digits + most_significant_bit(integer[msi]);
				msi = isz - v.most_significant_index() - 1;
				int msb_y = (isz - msi - 1) * std::numeric_limits<T>::digits + most_significant_bit(v.integer[msi]);
				...
				while (true)
				{
					(*quotient_tmp).set_bit(msb_x - msb_y - bit);
					(*mul_result).mul(*quotient_tmp);
					if (*mul_result < *this || *mul_result == *this)
						break;

					bit++;
					(*quotient_tmp).integer.clear();
					*mul_result = v;
				}
		...
		bdig& div2 (const bdig& v, bdig *premainder = NULL)
		{
			...
			int v_bits = (isz - v.most_significant_index() - 1) * std::numeric_limits<T>::digits + most_significant_bit (v.integer[v.most_significant_index()]);
			...
			(*quotient).is_negative = is_negative != v.is_negative;
			(*remainder).is_negative = is_negative != v.is_negative;
...
private:
		...
		template <class Ti>
		bdig& mul (const bdig& v, typename enable_if<
			...
			, Ti>::type = 0)
		{
			...
			for (int v_i = isz - 1; v_i >= v.most_significant_index(); v_i--)
			{
				...
				for (int idx = isz - 1; idx >= most_significant_index(); idx--)
				{
					...
					upT y = integer[idx];
					upT d = x * y;
					Ti r1 = d >> std::numeric_limits<T>::digits;
					Ti r0 = d & std::numeric_limits<T>::max();
			...
			for (int i = (*multiplicant).most_significant_index (); i < isz; i++ )
			{
				for (int b = 0; b < std::numeric_limits<T>::digits; b++)
				{
					if ((*multiplicant).integer[isz - 1] & (Ti)1)
					{
						...
						{
							if (shift)
								set_bit = (*sum).shl_sop (i, shift, set_bit);
							c = sum_sop (i, *sum, c);
						}
						shift = 0;
					}
		...
		bool _Cmp (const bdig& v1, const bdig& v2, bool r1, bool r2) const
		{
			...
			//int res1 = memcmp (v1.integer, v2.integer, sizeof (v1.integer)); // Can work only with big-endian 
			// v1 < v2
			if (r1)
				return (res < 0);
			// v1 > v2
	...
	public:
		...
		////////////////////////////////////
		// Binary operators
		template <class Ti> typename enable_if<std::numeric_limits<Ti>::is_integer, bdig>::type  operator + (const Ti v) const
		{
			return *this + bdig(v);
		}
		...
		template <class Ti> typename enable_if<std::numeric_limits<Ti>::is_integer, bdig>::type  operator % (const Ti v) const
		{
			...
			if (most_significant_index() == isz - 1 && std::numeric_limits<T>::digits >= std::numeric_limits<Ti>::digits)
		...
		template <class Ti> typename enable_if<std::numeric_limits<Ti>::is_integer, bool>::type operator > (const Ti v) const
		{
			return *this > bdig(v);
		}
		...
		template <class Ti> const typename enable_if<std::numeric_limits<Ti>::is_integer, bool>::type  operator == (const Ti v) const
		{
			return *this == bdig(v);
		}
		bool operator == (const bdig& v) const
		...
		template <class Ti> typename enable_if<std::numeric_limits<Ti>::is_integer && std::numeric_limits<Ti>::is_signed, bdig>::type& operator = (Ti v)
		{
			if (v < 0)
				is_negative = true;
		...
		{
			integer.clear();
			const int Ti_digits = std::numeric_limits<Ti>::digits + (std::numeric_limits<Ti>::is_signed ? 1 : 0);
			const int locate_size = (Ti_digits >= std::numeric_limits<T>::digits ? Ti_digits / std::numeric_limits<T>::digits : std::numeric_limits<T>::digits / Ti_digits);
			int idx_to = isz > locate_size ? isz - locate_size : 0;
			for (int i = isz - 1; i >= idx_to; i--)
			{
				integer.set(i, v & (T)-1);
				v = v >> (std::numeric_limits<T>::digits < std::numeric_limits<Ti>::digits ? std::numeric_limits<T>::digits - 1 : std::numeric_limits<Ti>::digits - 1);
				v = v >> 1;
			}
			return this->prec_up();
		}
		...
		friend std::ostream& operator << (std::ostream& output, const bdig& rhs)
		{
			...
			//output.write ((std::ostream::char_type *)&rhs.integer[msi], (isz - msi) * sizeof(T) / sizeof (std::ostream::char_type));
			return output;
		}
		void store (const char *name)
		{
			...
			try
			{
				...
				out.write ((std::ostream::char_type *)&integer[0], isz * sizeof(T) / sizeof (std::ostream::char_type));
				out.close();
			}
		...
		void load (const char *name)
		{
			...
			try
			{
				...
				in.read((std::ifstream::char_type *)integer, isz * sizeof(T) / sizeof (std::ifstream::char_type));
				in.close();
			}
			catch (std::ios_base::failure f)
			{
				std::cout << "Caught an exception: " << f.what() << " char_type:" << sizeof (std::ifstream::char_type) << std::endl;
				std::cout << " offset:" << (int)(isz * sizeof(T) / sizeof (std::ifstream::char_type)) << std::endl;
				//cout << " pos:" << (std::streamoff)in.tellg() << endl;
			}
		}
		*/

		template <class Ti> friend typename sag::enable_if <std::numeric_limits<Ti>::is_integer /*|| std::numeric_limits<Ti>::is_iec559*/, bdig>::type operator + (const Ti v1, bdig& v2) 
		{
			return (v2 + v1);
		}
		template <class Ti> friend typename sag::enable_if <std::numeric_limits<Ti>::is_integer /* || std::numeric_limits<Ti>::is_iec559*/, bdig>::type operator - (const Ti v1, bdig& v2) 
		...
		template <class Ti> friend typename sag::enable_if <std::numeric_limits<Ti>::is_integer, bdig>::type operator * (const Ti v1, bdig& v2) 
		{
			return (v2 * v1);
		}
		template <class Ti> friend typename sag::enable_if <std::numeric_limits<Ti>::is_integer /* || std::numeric_limits<Ti>::is_iec559*/, bdig>::type operator / (const Ti v1, bdig& v2) 
